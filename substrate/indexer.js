    ...
    */
    ...
    async update_chain_assets(chain, daysago = 2) {
        try {
            ...
            let sql = `insert into blocklog (chainID, logDT, startBN, endBN, numTraces, numExtrinsics, numEvents, numTransfers, numSignedExtrinsics, valueTransfersUSD, numXCMTransfersIn, numXCMMessagesIn, numXCMTransfersOut, numXCMMessagesOut, valXCMTransferIncomingUSD, valXCMTransferOutgoingUSD ${eflds} ) (select ${chainID} as chainID, date(blockDT) as logDT, min(blockNumber), max(blockNumber), sum(if(crawlTrace=0, 1, 0)), sum(numExtrinsics), sum(numEvents), sum(numTransfers), sum(numSignedExtrinsics), sum(valueTransfersUSD), sum(numXCMTransfersIn), sum(numXCMMessagesIn), sum(numXCMTransfersOut), sum(numXCMMessagesOut), sum(valXCMTransferIncomingUSD), sum(valXCMTransferOutgoingUSD) ${evals} from block${chainID} where blockDT is Not Null and blockDT >= date(date_sub(now(), INTERVAL ${daysago} DAY))  group by chainID, logDT) on duplicate key update startBN = values(startBN), endBN = values(endBN), numExtrinsics = values(numExtrinsics), numEvents = values(numEvents), numTransfers = values(numTransfers), numSignedExtrinsics = values(numSignedExtrinsics), valueTransfersUSD = values(valueTransfersUSD), numTraces = values(numTraces), numXCMTransfersIn = values(numXCMTransfersIn), numXCMMessagesIn = values(numXCMMessagesIn), numXCMTransfersOut = values(numXCMTransfersOut), valXCMTransferIncomingUSD = values(valXCMTransferIncomingUSD), valXCMTransferOutgoingUSD = values(valXCMTransferOutgoingUSD) ` + eupds;
            this.batchedSQL.push(sql);
