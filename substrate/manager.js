...
module.exports = class Manager extends AssetManager {
    ...
    async updateChainlogNumAccountsActive(chainID = null, lookback = 30) {
        await this.assetManagerInit();
    ...
    async scan_active_signers(chainID, startBN, endBN, address)
    {
	...
	for ( let i = startBN; i <= endBN; i+= jmp) {
	    ...
	    await this.scan_active_signers(chainID, i0, i1, address);
    ...
    async updateChainlogNumAddresses(chainID = null, lookback = 30) {
        await this.assetManagerInit();
	let whereChain = ( chainID >= 0 ) ? ` and chainID = ${chainID}` : "";
